...
router.post('/ucicentro' , (req, res)=> { //TRAER INFORMACION DE LA BASE DE DATOS
    client.connect(async (err) =>{
        if(!err){ //Preguntamos si no hay error
            const collection = client.db("uci").collection("limacentro") //Conectamos a la base de datos y ala coleccion
            collection.find().toArray((err, result) => {
                if(!err){
                    res.render('ucicentro' , {datos:result})
                }else{
                    res.send("resultado: [{'respuesta' : 'Error al traer la data'}, {'mensaje' : " + err+ "}]")
                }
            })
